...
const VideoLibrary = () => {
  ...
  const VideoCard = ({ video }) => {
    const fileSize = video.fileSize ? Math.round(video.fileSize/1024/1024) + ' MB' : '0 MB';
    ...
    if (view === 'list') {
      return (
        <Link
          to={`/video/${video.id}`}
          className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-all duration-200 block"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <p className="text-sm text-blue-800">
          <strong>Tenant Isolated:</strong> You're viewing videos from tenant "{user?.tenantId}"
        </p>
      </div>
    </div>
  );
